...
function expense_categories_ensure_table(PDO $pdo): void
{
    $pdo->exec("CREATE TABLE IF NOT EXISTS finance_categories (
        ...
        expense_scope VARCHAR(20) NOT NULL DEFAULT 'shared',
        created_by VARCHAR(100) NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NULL DEFAULT NULL,
        KEY idx_finance_categories_type (type),
        KEY idx_finance_categories_scope (expense_scope),
        KEY idx_finance_categories_parent (parent_category)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    if (!expense_categories_table_has($pdo, 'finance_categories', 'expense_scope')) {
        ...
        $pdo->exec("ALTER TABLE finance_categories ADD KEY idx_finance_categories_scope (expense_scope)");
    }
}

function expense_categories_table_has(PDO $pdo, string $table, string $column): bool
{
...
function expense_categories_scope(?string $value): string
{
...
{
    $mainStmt = $pdo->query("
        SELECT id, name, expense_scope, created_at
    ...
    ");
    $subStmt = $pdo->query("
        SELECT child.id, child.name, child.parent_category, COALESCE(parent.expense_scope, 'shared') AS expense_scope
        FROM finance_categories child
        LEFT JOIN finance_categories parent ON parent.type = 'main' AND parent.name = child.parent_category
        WHERE child.type = 'sub'
        ORDER BY child.parent_category, child.name
        LIMIT 3000
    ");
    ...
    return [
        ...
        'mainCategories' => array_map(static function (array $row): array {
            return [
                ...
                'expense_scope' => expense_categories_scope((string)($row['expense_scope'] ?? 'shared')),
...
try {
    ...
    if ($action === 'add_main_category') {
        ...
        $scope = expense_categories_scope($_POST['expense_scope'] ?? 'shared');
        ...
        $stmt = $pdo->prepare("INSERT INTO finance_categories (name, type, expense_scope, created_by) VALUES (?, 'main', ?, ?)");
        $stmt->execute([$name, $scope, expense_categories_user_name()]);
    ...
    if ($action === 'edit_main_category') {
        ...
        $pdo->prepare("UPDATE finance_categories SET name = ?, expense_scope = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND type = 'main'")
            ->execute([$name, $scope, $id]);
        if ($oldName !== '') {
            $pdo->prepare("UPDATE finance_spending SET category = ? WHERE category = ?")->execute([$name, $oldName]);
            $pdo->prepare("UPDATE finance_categories SET parent_category = ? WHERE parent_category = ? AND type = 'sub'")->execute([$name, $oldName]);
        }
