...
function marketing_filter_dates(array $filters): array
{
    $from = marketing_date($filters['date_from'] ?? $filters['from'] ?? null, date('Y-m-d', strtotime('-30 days')));
    $to = marketing_date($filters['date_to'] ?? $filters['to'] ?? null, date('Y-m-d'));
    if ($from > $to) {
        [$from, $to] = [$to, $from];
    }
    return [$from, $to];
...
{
    [$from, $to] = marketing_filter_dates($filters);
